...
$(document).ready(function () {
    ...
    loadModule(['dataTable'], function () {
        var consultationTbl = $('#consultation');
        consultation_table = consultationTbl.DataTable($.extend(true,{}, dtAutoOption(consultationTbl), {
            ...
            ajax: function (data, callback, settings) {
                var query_data = {};
                var query_form = $("#consultation_form").serializeArray();
                ...
                $.getJSON(consultation_url, toEnableParams(data, query_data), function (resp) {
                    callback(resp);
                });
            },
            columns: [
...
//                $('td:eq(1)', nRow).html('<img style="width:60px;" src="'+root_path_url+'/'+aData.top_img+'">');
        ...
        $("#consultation_btn").click(function(){consultation_table.ajax.reload(null, false)});
        consultationTbl.on('click','[data-action]',function(){
            ...
            if(action == 'detail'){
                $.ajax({
                    url:consultation_judge_url,
                    data:{id:id,model:'Consultation',action:'detail'},
...
$("#consultation").on('click','.js-stop',function(){
    var _this = $(this);
    Dialog.confirm('操作提示', '确定取消发布该资讯吗？', function () {
        var id = _this.attr("data-id");
        $.ajax({
            url: consultation_stop_url,
            type: 'post',
            data: {id: id},
            dataType: 'json',
            success: function (resp) {
...
$("#consultation").on('click','.js-start',function(){
    var _this = $(this);
    Dialog.confirm('操作提示', '确定发布该资讯吗？', function () {
        var id = _this.attr("data-id");
        $.ajax({
            url: consultation_start_url,
            type: 'post',
            data: {id: id},
            dataType: 'json',
            success: function (resp) {
...
$("#consultation").on('click','.js-hot-true',function(){
    var _this = $(this);
    Dialog.confirm('操作提示', '确定置顶该资讯吗？', function () {
        var id = _this.attr("data-id");
        $.ajax({
            url: consultation_hot_true_url,
            type: 'post',
            data: {id: id},
            dataType: 'json',
            success: function (resp) {
...
$("#consultation").on('click','.js-hot-false',function(){
    var _this = $(this);
    Dialog.confirm('操作提示', '确定取消置顶该资讯吗？', function () {
        var id = _this.attr("data-id");
        $.ajax({
            url: consultation_hot_false_url,
            type: 'post',
            data: {id: id},
            dataType: 'json',
            success: function (resp) {
...
$("#consultation").on('click','.js-delete',function(){
    var _this = $(this);
    Dialog.confirm('操作提示', '确定删除该资讯吗？', function () {
        var id = _this.attr("data-id");
        $.ajax({
            url: consultation_delete_url,
            type: 'post',
            data: {id: id},
            dataType: 'json',
            success: function (resp) {
