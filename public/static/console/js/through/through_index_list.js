    ...
    loadModule(['dataTable'], function () {
        ...
        through_table = throughTbl.DataTable($.extend(true,{}, dtAutoOption(throughTbl), {
          ...
          /*  "fnRowCallback": function (nRow, aData, iDisplayIndex, fnRowCallback) {
                $('td:eq(9)', nRow).html("<div class='badge bg-color-blue'>未审核</div>");
                ...
                $('td:eq(10)', nRow).html('<a href="javascript:void(0);" class="btn btn-xs btn-info cale" data-action="detail" data-id="' + aData.t_id + '"><i class="fa fa-search"></i>查看详情</a>');
                $('td:eq(10)', nRow).append('<a href="javascript:void(0);" data-id="' + aData.t_id + '" class="btn btn-xs btn-danger js-stop cale">终止</a>');
                return nRow;
            }
        }));*/
        "fnRowCallback": function (nRow, aData, iDisplayIndex, fnRowCallback) {
                ...
                $('td:eq(7)', nRow).html("<div class='badge bg-color-blue'>未审核</div>");
