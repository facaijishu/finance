...
$(document).ready(function () {
    ...
    loadModule(['dataTable'], function () {
        var newsTbl = $('#news');
        news_table = newsTbl.DataTable($.extend(true,{}, dtAutoOption(newsTbl), {
            ...
            ajax: function (data, callback, settings) {
                var query_data = {};
                var query_form = $("#news_form").serializeArray();
                ...
                $.getJSON(news_url, toEnableParams(data, query_data), function (resp) {
                    callback(resp);
                });
            },
            columns: [
        ...
        $("#news_btn").click(function(){news_table.ajax.reload(null, false)});
        newsTbl.on('click','[data-action]',function(){
            var self = $(this);
            ...
            if(action == 'detail'){
                $.ajax({
                    url:news_judge_url,
                    data:{id:id,model:'News',action:'detail'},
...
$("#news").on('click','.js-stop',function(){
    var _this = $(this);
...
$("#news").on('click','.js-start',function(){
    var _this = $(this);
...
$("#news").on('click','.js-hot-true',function(){
    var _this = $(this);
    Dialog.confirm('操作提示', '确定置顶该资讯吗？', function () {
        var id = _this.attr("data-id");
        $.ajax({
            url: news_hot_true_url,
            type: 'post',
            data: {id: id},
            dataType: 'json',
            success: function (resp) {
...
$("#news").on('click','.js-hot-false',function(){
    var _this = $(this);
    Dialog.confirm('操作提示', '确定取消置顶该资讯吗？', function () {
        var id = _this.attr("data-id");
        $.ajax({
            url: news_hot_false_url,
            type: 'post',
            data: {id: id},
            dataType: 'json',
            success: function (resp) {
